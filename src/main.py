...
def load(ds, shuffle=False):
    ...
    def collate_fn(data):
        ...
        return (
            input.to(cfg.device),
            label.to(cfg.device),
            torch.tensor(input_lengths, device=cfg.device),
            torch.tensor(label_lengths, device=cfg.device),
        )
...
def train_epoch(model, ds, optimizer):
    model.train()
    for input, label, input_lengths, label_lengths in load(ds):
        optimizer.zero_grad()
        logits = model(input, label, input_lengths, label_lengths)
...
def evaluate(model, ds):
    ...
    with torch.no_grad():
        loss = 0
        errors = 0
        for input, label, input_length, label_length in load(ds):
            logits = model(
                input,
                label,
                input_length,
...
def main():
    ...
    model = EncoderDecoder(encoder, decoder, device=cfg.device)
